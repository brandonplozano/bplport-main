...

{
  ...
  return <div className="flex flex-col ml-auto mr-auto items-center justify-center w-[1200px] text-white gap-6 mt-[10rem] mb-[3rem]">
          <div className={`p-6 bg-[#262626] text-white text-[1.5rem] w-[75%] font-medium text-center rounded-lg shadow-xl ${styles.customGradient}`}>
            [CAPSTONE] MyT-Pass: Web Based Training Record and Document Management System for Manufacturing Businesses in the Philippines</div>
          <div className="flex flex-wrap cursor-pointer">
            {imageArray.map((image, idx) => (
                <ImageCard h={1000}key={idx} image={image} onClick={() => onClick(image)} />
            ))}
          </div>
